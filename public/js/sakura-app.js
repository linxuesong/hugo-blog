...
function code_highlight_style () {
  // 根据代码块语言类型生成顶部条幅 如go 代码块顶部显示Golang 需要注册对应语言
  ...
  // 所有代码块pre下的code标签 样式处理 
  $('pre code').each(function (i, block) {
    hljs.highlightBlock(block)
  })

  // 生成顶部条幅
  // 获取article标签下所有的pre标签 pre标签就是代码块的标签
  ...
  hljs.initLineNumbersOnLoad() // 调用lib.min.js的函数生成代码行号
}
try {
    ...
    for (var i = 0; i < this.files.length; i++) {
      ...
      $.ajax({
        ...
        beforeSend: function (xhr) {
          ...
          $('#upload-img-show').append('<img class="lazyload upload-image-preview" src="/imgs/loader/trans.ajax-spinner-preloader.svg" data-src="' + get_the_url + '" onclick="window.open(\'' + get_the_url + '\')" onerror="imgError(this)" />')
          lazyload()
...
function add_copyright () {
  document.body.addEventListener('copy', function (e) {
    if (mashiro_option.copy_right.enable && !mashiro_global.is_user_logged_in && window.getSelection().toString().length > mashiro_option.copy_right.enable) {
      setClipboardText(e)
    }
  })
  ...
  function setClipboardText (event) {
    ...
    var htmlData = ""
    var textData = ""
    htmlData = '' + '著作权归作者所有。<br>' + '商业转载请联系作者获得授权，非商业转载请注明出处。<br>' + '作者：' + mashiro_option.author_name + '<br>' + '链接：' + window.location.href + '<br>' + '来源：' + mashiro_option.site_name + '<br><br>' + window.getSelection().toString().replace(/\r\n/g, '<br>')
...
var home = location.href,
  ...
  Siren = {
    ...
    MN: function () {
      ...
      // 分页逻辑: 点一次previous按钮 加入下一页内容 
      // 1.将layouts/index.html中pagination的href做请求获取下一页 2. 取出id=main的文章部分div 3.插入到文章展示页下面
      // page/2/#main中 #mian仅是将页面滚动到id=mian的div作用,可删去url中的#main
